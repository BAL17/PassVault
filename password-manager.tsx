...
import { Search, Eye, EyeOff, Copy, Plus, Trash, Lock, ExternalLink, Star, User, LogIn, FileText, ChevronDown, Menu } from 'lucide-react'
...
export default function PasswordManager() {
  ...
  const filteredPasswords = passwords.filter(entry => {
    ...
    const categoryMatch = 
      ...
      activeCategory === "favorites" ? entry.favorite :
      activeCategory === "logins" ? entry.type === "login" :
      activeCategory === "notes" ? entry.type === "note" : 
      true;
  ...
  const toggleFavorite = (id: number) => {
    setPasswords(passwords.map(password => 
      password.id === id ? { ...password, favorite: !password.favorite } : password
    ))
  }
  ...
  const getCategoryCount = (category: Category) => {
    switch (category) {
      case "all": return passwords.length
      case "favorites": return passwords.filter(p => p.favorite).length
      case "logins": return passwords.filter(p => p.type === "login").length
      case "notes": return passwords.filter(p => p.type === "note").length
      default: return 0
    }
  }
        ...
        <div className="flex-1 w-full">
          <div className="w-full py-6 px-4 md:px-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Dialog open={isAddPasswordOpen} onOpenChange={setIsAddPasswordOpen} className="overflow-visible">
                ...
                <DialogContent className="max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>Add New {activeCategory === "notes" ? "Secure Note" : "Password"}</DialogTitle>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                      ...
                      <>
                        ...
                        <div className="grid gap-2">
                          ...
                          {newPassword.password && (
                            <div className="mt-1">
                              <div className="text-sm mb-1">Password Strength:</div>
                              <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                                <div 
                                  className={`h-full ${getStrengthColor(calculatePasswordStrength(newPassword.password))}`} 
                                  style={{ 
                                    width: calculatePasswordStrength(newPassword.password) === "weak" 
                                      ? "33%" 
                                      : calculatePasswordStrength(newPassword.password) === "medium" 
                                        ? "66%" 
                                        : "100%" 
                                  }}
                                />
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="notes">{activeCategory === "notes" ? "Secure Note" : "Notes (Optional)"}</Label>
                      <Input
                        id="notes"
                        placeholder={activeCategory === "notes" ? "Enter your secure note" : "Additional information"}
                  ...
                  <div className="flex justify-end">
                    <Button onClick={handleAddPassword}>Save {activeCategory === "notes" ? "Note" : "Password"}</Button>
                  </div>
                </DialogContent>
              </Dialog>
            </div>
              ...
              {filteredPasswords.length === 0 ? (
                <Card className="w-full">
                  <CardContent className="py-10 text-center">
                    <p className="text-muted-foreground">No items found. Add a new {activeCategory === "notes" ? "note" : "password"} to get started.</p>
                  </CardContent>
                </Card>
              ) : (
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <CardDescription>Last updated: {entry.lastUpdated}</CardDescription>
                              </div>
                            </div>
                            <Badge 
                          ...
                          <div className="grid gap-3">
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="font-medium font-mono">
                                  {showPasswordId === entry.id ? entry.password : "••••••••••••"}
                                </div>
                                <Button 
                                  variant="ghost" 
                                  size="icon" 
                            ...
                            {entry.notes && (
                              <div>
                                ...
                                <div className="text-sm bg-muted p-2 rounded">{entry.notes}</div>
                              </div>
                            )}
                          </div>
                        </CardContent>
